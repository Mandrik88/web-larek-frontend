...
- modal: open - событие инициируется при нажатии на карточку товара и ее открытии в модельном окне
- basket: order - событие, которое инициируется при клике по корзине, при этом открывается модальное окно с заказом
- basket: add - событие, которое инициируется при клике на кнопку "В корзину", при этом добавляет товар в корзину и вносит изменения в счетчик корзины на главной странице
...
- basket: changed - событие, которое инициируется в момент добавления/удаления товара в/из корзины, а также при очистке корзины
...
- getProducts(): IProduct[] - метод получения католога с товарами
